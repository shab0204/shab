...
function generatePosts(arrPosts){
    rootElem.innerHTML = ''
    arrPosts.forEach(element => {

        const post = document.createElement('div')
        post.className = `post_userId`

        const client = document.createElement('p')
      client.className = `client_postId`
      client.textContent = `userId: ${element.userId}`

      const titleId = document.createElement('h1')
      titleId.className = `user`
      titleId.textContent = `id: ${element.id}`

      const title = document.createElement('h3')
      title.className = `title_userId`
      title.textContent = element.title

      const userIdd = document.createElement('h2')
      userIdd.className = `body_userId`
      userIdd.textContent = element.body

    
      post.appendChild(client);
post.appendChild(titleId);
post.appendChild(title);
post.appendChild(userIdd);

rootElem.appendChild(post)
    });
